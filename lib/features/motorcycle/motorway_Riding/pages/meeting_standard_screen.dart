...
class _MeetingStandardScreenRidingState extends State<MeetingStandardScreenRiding> {
  ...
  void initState() {
    SchedulerBinding.instance.addPostFrameCallback((_) {
      final provider = Provider.of<MotorwaySafetyNotifier>(context, listen: false);
      provider.fetchMotorwaySafety();
    });
    super.initState();
  }
  @override
  Widget build(BuildContext context) {
    return  Scaffold(
      ...
      body: Consumer<MotorwaySafetyNotifier>(
          builder: (context,provider,child) {
            final data = provider.motorwaySafety;
            if (data == null) {
              return const Center(
                child: LoadingScreen(),
              );
