...
class CarryingPassengerScreen extends StatefulWidget {
  const CarryingPassengerScreen({super.key});

  @override
  State<CarryingPassengerScreen> createState() =>
      _CarryingPassengerScreenState();
}

class _CarryingPassengerScreenState extends State<CarryingPassengerScreen> {
  ...
  void initState() {
    SchedulerBinding.instance.addPostFrameCallback((_) {
      final provider =
          Provider.of<LorryCarryingPassengerProvider>(context, listen: false);
  ...
  void category() async {
    final provider =
        Provider.of<LorryCarryingPassengerProvider>(context, listen: false);
    provider.saveCategoryType("carrying passenger");
  }

  void totalQuestion(int total) async {
    final provider =
        Provider.of<LorryCarryingPassengerProvider>(context, listen: false);
    provider.saveTotalQuestionIndex(total);
  }

  void correctAnswer(List<String> correctAnswers) {
    final provider =
        Provider.of<LorryCarryingPassengerProvider>(context, listen: false);
    provider.saveCorrectAnswerList(correctAnswers);
  }

  void wrongAnswer(List<String> wrongAnswers) {
    final provider =
        Provider.of<LorryCarryingPassengerProvider>(context, listen: false);
    provider.saveWrongAnswerList(wrongAnswers);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Consumer<LorryCarryingPassengerProvider>(
                ...
                Padding(
                  ...
                  child: ElevatedButton(
                    onPressed: _currentIndex < provider.lorryModels.length - 1
                      ...
                      final provider = Provider.of<LorryCarryingPassengerProvider>(context, listen: false);
                      final correctAnswer = await provider.getCorrectAnswerList();
                      final wrongAnswers = await provider.getWrongAnswerList();
                      final currentQuestionIndex = _currentIndex;
                      final totalQuestion = await provider.getTotalQuestionIndex();
                      final category = await provider.getCategoryType();
