...
class DrivingHourRestPeroidScreen extends StatefulWidget {
  const DrivingHourRestPeroidScreen({super.key});

  @override
  State<DrivingHourRestPeroidScreen> createState() => _DrivingHourRestPeroidScreenState();
}

class _DrivingHourRestPeroidScreenState extends State<DrivingHourRestPeroidScreen> {
  ...
  void initState() {
    SchedulerBinding.instance.addPostFrameCallback((_) {
      final provider = Provider.of<LorryDriverHoursRestProvider>(context, listen: false);
  ...
  void category() async {
    final provider = Provider.of<LorryDriverHoursRestProvider>(context, listen: false);
    provider.saveCategory("Driver Hours Rest Period");
  }
  ...
  Widget build(BuildContext context) {
    return  Scaffold(
      appBar: CustomAppBar(title: "Driver hours Rest Period", leadingIcon: Icons.arrow_back, onLeadingIconPressed: (){}),
           body: Center(
    child: Consumer<LorryDriverHoursRestProvider>(
            ...
            child: ElevatedButton(
              onPressed: _currentIndex < provider.lorryModels.length - 1
                ...
                _pageController.nextPage(
                  ...
                  : () async {
                final provider = Provider.of<LorryDriverHoursRestProvider>(
                    context,
                    listen: false);
                final correctAnswer =
                await provider.getCorrectAnswers();
                final wrongAnswers =
                await provider.getWrongAnswers();
