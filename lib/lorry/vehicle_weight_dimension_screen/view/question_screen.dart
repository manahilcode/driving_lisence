...
class VehicleWeightDimensionScreen extends StatefulWidget {
  const VehicleWeightDimensionScreen({super.key});

  @override
  State<VehicleWeightDimensionScreen> createState() =>
      _VehicleWeightDimensionScreenState();
}

class _VehicleWeightDimensionScreenState
    extends State<VehicleWeightDimensionScreen> {
  ...
  void initState() {
    SchedulerBinding.instance.addPostFrameCallback((_) {
      final provider = Provider.of<LorryVehicleWeightDimensionProvider>(context,
          listen: false);
  ...
  void category() async {
    final provider = Provider.of<LorryVehicleWeightDimensionProvider>(context,
        listen: false);
    provider.saveCategoryType("Vehicle Weight Dimension");
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Consumer<LorryVehicleWeightDimensionProvider>(
                  ...
                  child: ElevatedButton(
                    onPressed: _currentIndex < provider.lorryModels.length - 1
                      ...
                      _pageController.nextPage(
                        ...
                        : () async {
                      final provider =
                      Provider.of<LorryVehicleWeightDimensionProvider>(
                          context,
                          listen: false);
                      final correctAnswer =
                      await provider.getCorrectAnswerList();
                      final wrongAnswers =
                      await provider.getWrongAnswerList();
                      final currentQuestionIndex = _currentIndex;
                      final totalQuestion =
                      await provider.getTotalQuestionIndex();
